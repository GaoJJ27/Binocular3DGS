...
def readImages(renders_dir, gt_dir):
    ...
    image_names = []
    # 获取文件名列表并按照数字顺序排序
    fnames = sorted(os.listdir(renders_dir), key=lambda x: int(x.split('.')[0]))
    ...
    for scene_dir in model_paths:
        try:
            ...
            for method in os.listdir(test_dir):
                ...
                elif dataset_name == "siga":
                    print("eval siga with mask ...")
                    masks = []
                    for image_name in image_names:
                        mask_name = image_name
                        mask_image_path = os.path.join(idrmasks_path, mask_name)
